...
class GameStart extends Component {
    ...
    stay() {
        if (this.state.roundOver !== 'BUST!') {
            ...
            if (+this.state.dealersTotal + +this.state.dealersHiddenValue > 16) {
                console.log('test')
            }
            this.dealNewCard();
        }
    ...
    dealNewCard() {
        if (this.state.dealersTotal < 17 && this.state.playersTotal < 22) {
            axios({
                url: 'https://deckofcardsapi.com/api/deck/' + this.props.deck_id + '/draw/?count=1',
                method: 'GET',
                responseType: 'JSON',
    ...
    newGame() {
        ...
        axios({
            ...
            console.log(this.state.dealersHiddenCard)
